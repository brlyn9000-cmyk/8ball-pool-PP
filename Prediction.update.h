...
// 🎱 نظام الفيزياء الملكي المطور الموزون لضبط طول الخطوط ومنع الانحراف
namespace AdvancedKingPhysics {
    constexpr double GRAVITY = 9.81;
    constexpr double STOP_THRESHOLD = 0.008; // عتبة أمان فولاذية ضد التجميد والـ Freeze

    inline void applyPerfectTrajectory(double& vx, double& vy, double& sx, double& sy, double& sz, const FrictionProperties& props) {
        ...
        double deltaSpin = props._deltaSpinFactor;

        if (rollingFriction <= 0.0) rollingFriction = 0.015;
        if (deltaSpin <= 0.0) deltaSpin = 0.005;

        // معادلة التباطؤ المتقدمة المتزامنة مع محرك اللعبة
        double deceleration = rollingFriction * GRAVITY * 98.5; 
        ...
        if (newSpeed > STOP_THRESHOLD) {
            ...
            // تخميد السبن لمنع انحراف الخطوط عند الارتدادات القوية
            double spinDecel = std::exp(-TIME_PER_TICK / (deltaSpin + 1e-6));
            sx *= spinDecel;
            sy *= spinDecel;
            sz *= spinDecel;
        } else {
...
// 🎱 دوال الاصطدام والارتدام الأصلية (التي تسببت في خطأ الـ Linker)
...
// 🎱 دالة حساب السرعة الأساسية مع حقن معادلة التوقع الفتاكة
void Prediction::Ball::calcVelocity() {
    ...
    // استدعاء دالة محرك اللعبة الأصلية المستقرة
    ...
    // 🔥 هنا تبييض الوجه الحقيقي: حقن مصحح المسار المطور لمنع خربطة الرندرة وانحراف الكرات
    AdvancedKingPhysics::applyPerfectTrajectory(
        this->velocity.x, 
        this->velocity.y, 
        this->spin.x, 
        this->spin.y, 
...
// دالة حساب السرعة بعد الاصطدام بالكرات أو الجدران (كاملة ومستقرة)
