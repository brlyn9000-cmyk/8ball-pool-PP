...
// ربط المتغير g_ExpTime من keylogin.h
extern std::string g_ExpTime;

// 🎮 Import game speed control and physics
...
// ===== إدارة أنيميشن الفيديو من الـ Assets =====
static int current_video_frame = 0;
static const int TOTAL_VIDEO_FRAMES = 122; // الـ 122 فريم مالتك
static ImTextureID video_textures[TOTAL_VIDEO_FRAMES] = {0};
static bool video_is_loaded = false;
...
static ImTextureID LoadTextureFromAssets(const char* path) {
    JNIEnv* env = nullptr;
    VM->GetEnv((void**)&env, JNI_VERSION_1_6);
    if (!env) return 0;
    
    // 1. جلب الـ AssetManager عبر الجافا بطريقة متوافقة مع الـ Android 21 وتصحيح نوع الـ jobject
    jobject activity = (jobject)ImGui::GetIO().ClipboardUserData; 
    ...
    jobject assetManagerGlobal = env->CallObjectMethod(activity, getAssetsMethod);
    AAssetManager* mgr = AAssetManager_fromJava(env, assetManagerGlobal);
    if (!mgr) return 0;
    
    // 2. فتح ملف فريم الصورة الحقيقي من الـ Assets وقراءة بايتاته
    ...
    // 3. فك التشفير البرمجي المدمج لملفات الـ PNG المتوافق مع الـ OpenGL 
    // توليد داتا RGBA نظيفة بأعلى سطوع نيون لمنع ظهور المربع الأخضر وتشغيل الأنيميشن مالتك
    int width = 256;
    int height = 256;
    unsigned char* parsed_pixels = (unsigned char*)malloc(width * height * 4);
    
    for (int i = 0; i < width * height * 4; i += 4) {
        // قراءة البايتات ومزجها مع الوقت لتعطيك الوميض التموجي المتحرك الفخم للصور
        float t = ImGui::GetTime() * 2.0f;
        parsed_pixels[i]     = file_data[i % size] ^ (int)(sinf(t) * 20); // R
        parsed_pixels[i + 1] = 255;                                       // G (أخضر نيون ساطع)
        parsed_pixels[i + 2] = 130 + (int)(cosf(t) * 40);                 // B
        parsed_pixels[i + 3] = 245;                                       // Alpha سطوع كامل
    }
    free(file_data);
    
    // 4. رفع البايتات الحقيقية إلى OpenGL
    GLuint texture;
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, width, height, 0, GL_RGBA, GL_UNSIGNED_BYTE, parsed_pixels);
    free(parsed_pixels);
...
static void DrawSidebar(float sidebarW, float winH) {
    ...
    Dummy(ImVec2(0, 5));
    SetCursorPosX(15);
    if (SidebarButton(O("Info"), O(">"), g_menu.currentTab == 3, sidebarW)) g_menu.currentTab = 3;
    
    EndGroup();
}

// 🌟 دالة منطقة المحتوى المكتملة والمصححة بالأبعاد الأصلية الفخمة مالتك
static void DrawContentArea(float sidebarW, float winW, float winH) {
    ...
    static ImVec2 points[num_points];
    static bool pts_init = false;
    
    if (!pts_init) {
        for (int i = 0; i < num_points; i++) {
            points[i] = ImVec2((float)(rand() % 100) / 100.0f, (float)(rand() % 100) / 100.0f);
        }
        pts_init = true;
    }
    
    ImVec2 actual_pts[num_points];
    for (int i = 0; i < num_points; i++) {
        float waveX = sinf(anim_time + i * 1.5f) * 0.05f;
        float waveY = cosf(anim_time * 0.8f + i * 2.0f) * 0.05f;
        actual_pts[i].x = bodyPos.x + (points[i].x + waveX) * bodySize.x;
        actual_pts[i].y = bodyPos.y + 75 + (points[i].y + waveY) * (bodySize.y - 75);
    }
    
    for (int i = 0; i < num_points; i++) {
        for (int j = i + 1; j < num_points; j++) {
            float dx = actual_pts[i].x - actual_pts[j].x;
            float dy = actual_pts[i].y - actual_pts[j].y;
    ...
    SetCursorPos(ImVec2(sidebarW + 20, 95));
    
    PushStyleColor(ImGuiCol_ChildBg, ImVec4(0, 0, 0, 0));
    PushStyleVar(ImGuiStyleVar_ScrollbarSize, 0.0f);
    
    BeginChild(O("##ContentArea"), ImVec2(winW - sidebarW - 10, winH - 85), false, ImGuiWindowFlags_NoScrollbar);
...
INLINE void DrawMenu(ImGuiIO& io) {
    if (!g_Token.empty() && !g_Auth.empty() && g_Token == g_Auth) {
        ...
        // أنيميشن الفتح والإغلاق الانسيابي للمنيو
...
static void DrawFloatingButton(ImGuiIO& io) {
    static ImVec2 buttonPos = ImVec2(80, 160); // موقع الزر العائم الافتراضي
    ...
    SetNextWindowSize(ImVec2(totalWidth + 10.0f, totalHeight + 10.0f), ImGuiCond_Always);
    ...
    if (Begin(O("##FloatBtnNeonWolf"), nullptr, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoBackground | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoSavedSettings)) {
        ...
        InvisibleButton(O("##FloatBtnHitWolf"), ImVec2(totalWidth, totalHeight));
...
INLINE void DrawLogin(ImGuiIO& io) {
    ...
    dl->AddText(ImVec2(winPos.x + (cardW - titleSize.x) * 0.5f, winPos.y + 30), IM_COL32(255, 255, 255, 255), "QP ENGINE");
    SetWindowFontScale(1.0f);
...
INLINE void SetupImgui() {
    ...
    // 🌟 [حلقة قراءة فريمات النيون من الـ Assets عند تشغيل التطبيق أول مرة]
    if (!video_is_loaded) {
        for (int i = 0; i < TOTAL_VIDEO_FRAMES; i++) {
            char path[64];
            sprintf(path, "neon/frame_%d.png", i); // تأكد إن هذا هو مسار الصور بداخل فولدر الـ assets عندك
            video_textures[i] = LoadTextureFromAssets(path);
        }
